...
class View {
        ...
        if (this.currentSearchValue) {
          show = show &&
            (item.title.toLowerCase().indexOf(this.currentSearchValue) !== -1 ||
              item.author.toLowerCase().indexOf(this.currentSearchValue) !==
              -1);
        }
        return show;
      });
  ...
  renderActionsCell(item) {
    ...
    if (item.free) {
      ...
      const subscribeButtonHTML = `<a href="${this.getSubscribeUrl(
        item.id)}" class="book-list__subscribe-button" target="_blank">${BUTTON_LABEL}</a>`;
  ...
  renderBookList(bookList) {
    ...
    const booksToRender = bookList.slice(startBookIndex,
      startBookIndex + PAGE_SIZE);
