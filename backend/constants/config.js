const prodConfig = {
  REGION: 'eu-central-1',
  SUBSCRIBER_LAMBDA: 'shellf-bot-dev-subscriber',
  NOTIFIER_LAMBDA: 'shellf-bot-dev-notifier',
  CLIENT_EMAIL: 'shellf_client_email',
  CLIENT_PRIVATE_KEY: 'shellf_private_key',
  SCOPE: 'https://www.googleapis.com/auth/spreadsheets',
  SHEETS_ID: '1_M9hvCTY1MAbxKIa2ANeUYQpnx2W8IPwleSaR4hazYs',
  ADMIN_CHAT_ID: 124760667,
  SUPPORT_CHAT_ID: 6116111170,
  BOOKS_DB: 'список книг',
  BOOKS_LOG: 'бот в телеге',
  BOOKS_RETURNED: 'вернулись',
  BOOKS_SUBS: 'подписки',

  BOOKID_COLUMN_DB: 1,
  TITLE_COLUMN_DB: 2,
  AUTHOR_COLUMN_DB: 3,
  STATUS_COLUMN_DB: 6,
  SHELF_COLUMN_DB: 7,

  DATE_COLUMN_LOG: 0,
  DEADLINE_COLUMN_LOG: 1,
  LANG_COLUMN_LOG: 2,
  USERNAME_COLUMN_LOG: 3,
  CHATID_COLUMN_LOG: 4,
  BOOKID_COLUMN_LOG: 5,
  TITLE_COLUMN_LOG: 6,
  AUTHOR_COLUMN_LOG: 7,
  SHELF_COLUMN_LOG: 8,
  PROLONG_COLUMN_LOG: 9,
  RETURN_COLUMN_LOG: 10,
  DUPLICATED_COLUMN_LOG: 11,
  SUBSCRIPTIONS_COLUMN_LOG: 12,
  SEQ_NUM_COLUMN_LOG: 13,
  LOG_COLUMNS_NUMBER: 14,

  DATE_COLUMN_SUBS: 0,
  LANG_COLUMN_SUBS: 1,
  USERNAME_COLUMN_SUBS: 2,
  CHATID_COLUMN_SUBS: 3,
  BOOKID_COLUMN_SUBS: 4,
  TITLE_COLUMN_SUBS: 5,
  AUTHOR_COLUMN_SUBS: 6,
  SHELF_COLUMN_SUBS: 7,
  UNSUBSCRIBE_COLUMN_SUBS: 8,
  EXPECTED_DATE_COLUMN_SUBS: 9,
  SEQ_NUM_COLUMN_SUBS: 10,
  SUBS_COLUMNS_NUMBER: 11,

  REMIND_DAYS: 3,
  OVERDUE_DAYS: 40,
  LOST_DAYS: 60,

  CATALOGUE_LINK: 'https://d30noal47qv51w.cloudfront.net/',
  TINKOFF_LINK: 'https://www.tinkoff.ru/collectmoney/crowd/anderson.layma1/latFj82826/?short_link=165fWSttnKA&httpMethod=GET',
  PAYPAL_LINK: 'https://www.paypal.com/paypalme/shelllf',
};

module.exports = prodConfig;

